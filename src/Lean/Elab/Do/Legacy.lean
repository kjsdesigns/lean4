...
def declToTerm (decl : Syntax) (k : Syntax) : M Syntax := withRef decl <| withFreshMacroScope do
  ...
  if kind == ``Parser.Term.doLet then
    -- Thread letConfig through to the term-level `let`
    -- Term `let` is: "let" >> letConfig >> letDecl >> optSemicolon >> termParser
    let declIdx := doLetDeclIdx decl
    if decl[2].isOfKind ``Parser.Term.letConfig then
      return mkNode `Lean.Parser.Term.«let» #[decl[0], decl[2], decl[declIdx], mkNullNode, k]
    else
      let letDecl := decl[declIdx]
