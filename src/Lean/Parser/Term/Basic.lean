...
Uses `checkColPosGt` so that the indentation requirement applies even at the top level where
no position has been saved. Falls back to an empty tactic sequence when no appropriately indented
content follows, producing an elaboration error (unsolved goals) rather than a parse error. -/
...
def tacticSeqIndentGt := withAntiquot (mkAntiquot "tacticSeq" ``tacticSeq) <| node ``tacticSeq <|
  tacticSeqBracketed <|> (checkColPosGt "indented tactic sequence" >> tacticSeq1Indented) <|>
